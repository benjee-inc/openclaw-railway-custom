  ...
  for (const m of markets) {
    ...
    if (m.change1h != null && Math.abs(m.change1h) >= 0.01 && m.volume24h > 3000) {
      ...
      signals.movers1h.push({
        q: m.question, dir, change1h: m.change1h, change1d: m.change1d,
    ...
    if (m.change1d != null && Math.abs(m.change1d) >= 0.03 && m.volume24h > 5000) {
      ...
      signals.movers1d.push({
        q: m.question, dir, change1d: m.change1d, change1w: m.change1w,
        price: yes, volume24h: m.volume24h,
    ...
    if (m.volume > 0 && m.volume24h > 10000) {
      ...
      if (ratio > 0.05) {
        signals.volumeSpikes.push({
          q: m.question, ratio, volume24h: m.volume24h, totalVol: m.volume,
          price: yes, change1h: m.change1h,
    ...
    if (m.endDate) {
      ...
      if (hrsLeft > 0 && hrsLeft < 72 && m.volume24h > 5000) {
        signals.catalysts.push({
          q: m.question, hrsLeft, price: yes, endDate: m.endDate,
          change1h: m.change1h, change1d: m.change1d, volume24h: m.volume24h,
    ...
    if (yes >= 0.005 && yes <= 0.10 && m.liquidity > 10000 && m.volume24h > 1000) {
      signals.deepValue.push({
        q: m.question, outcome: m.outcomes[0] || "Yes",
    ...
    if (yes >= 0.35 && yes <= 0.65 && m.competitive > 0.7 && m.volume24h > 10000) {
      signals.tightRaces.push({
        q: m.question, price: yes, competitive: m.competitive,
        volume24h: m.volume24h, change1h: m.change1h, change1d: m.change1d,
    ...
    if (m.spread > 0.03 && m.liquidity > 20000) {
      signals.wideSpread.push({
        q: m.question, spread: m.spread, bid: m.bestBid, ask: m.bestAsk,
...
async function runScanCycle() {
  ...
  try {
    ...
    const combined = [...existing, ...newEntries.map((e) => JSON.parse(e))];
    const trimmed = combined.slice(-MAX_ENTRIES);
    await pushJsonl(token, repo, sha, trimmed);
    console.log(`[scanner] pushed ${newEntries.length} entries (total: ${trimmed.length})`);
  } catch (err) {
